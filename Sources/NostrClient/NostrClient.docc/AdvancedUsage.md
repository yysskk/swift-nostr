...
let result = try await client.messages.send(
    "Hello privately!",
...
``NostrMessagesAPI/subscribe(limit:)`` delivers messages already unwrapped and parsed. A
...
For the raw gift-wrap events, use ``NostrMessagesAPI/giftWraps(limit:)``
and parse manually:

```swift
let giftWraps = try await client.messages.giftWraps()
for await giftWrap in giftWraps.events {
    let dm = try await client.messages.parse(giftWrap)
...
```swift
let dm = try await client.messages.parse(giftWrapEvent)
print("Message: \(dm.content)")
```

### Reactions (NIP-25)

React to a received message with ``NostrMessagesAPI/react(to:reaction:expiration:strategy:)``.
...
``NostrMessagesAPI/sendFile(url:mimeType:encryption:size:dimensions:blurhash:to:expiration:strategy:)``:
...
```swift
try await client.messages.send(
...
``NostrMessagesAPI/payloads(limit:)`` delivers messages, reactions, and file messages
...
DMs with ``NostrRoutingAPI/publishDirectMessageRelayList(relays:strategy:)`` (NIP-17 suggests 1–3
relays), connect your own inbox relays before receiving with
``NostrRoutingAPI/connectDirectMessageInboxRelays()``, and look up another user's DM relays with
``NostrRoutingAPI/fetchDirectMessageRelayList(for:timeout:)``:
...
let dmRelays = try await client.routing.fetchDirectMessageRelayList(for: "recipientPubkeyHex")
...
// A contact list is built and signed by the signer, then published like any event.
let signed = try signer.signContactList(contacts)
...
try await client.identity.setNsec("nsec1...")  // automatic from here on
```
...
```swift
for await event in try await client.subscriptions.subscribe(filters: [filter]) {
...
``NostrIdentityAPI/setSigner(_:)-(NostrSigning)`` accepts any ``NostrCore/NostrSigning`` — a local
...
public key is resolved once and cached, so ``NostrIdentityAPI/publicKey`` and ``NostrIdentityAPI/npub`` stay
synchronous.

Every feature runs through the signer, so a remote one is a drop-in for a local key: the generic
``NostrIdentityAPI/sign(_:)`` and ``NostrEventsAPI/publish(_:strategy:)``, the convenience `publish*`
...
try await client.identity.setSigner(signer)   // any NostrSigning

try await client.events.publishTextNote(content: "Signed by my bunker")
try await client.messages.send("and gift-wrapped by it too", to: recipientPubkey)
```

To author an event this library has no helper for, build an ``NostrCore/UnsignedEvent`` under
``NostrIdentityAPI/publicKey``, sign it, and publish it:

```swift
guard let pubkey = await client.identity.publicKey else { return }
...
``NostrGroupsAPI/join(_:inviteCode:reason:strategy:)`` applies the parsed code automatically
...
try await client.groups.leave(group)
```
...
``NostrGroupsAPI/timeline(_:kinds:since:limit:)`` follows the group's content —
...
``NostrGroupsAPI/fetchState(for:authorPubkey:timeout:)`` fetches a snapshot in one round
...
```swift
let state = try await client.groups.fetchState(for: group)
...
leaves. Unlike the group flows, ``NostrGroupsAPI/publishSimpleGroupList(_:strategy:)`` broadcasts
...
```swift
var list = try await client.groups.fetchSimpleGroupList() ?? SimpleGroupList()
...
try await client.groups.publishSimpleGroupList(list)
```
...
``NostrRoutingAPI/publishRelayList(read:write:strategy:)``, then let the client resolve and connect the
...
let relayList = try await client.routing.fetchRelayList(for: "pubkey...")
...
relays on demand with ``NostrRoutingAPI/subscribeOutbox(authors:kinds:limit:)``:

```swift
let outbox = try await client.routing.subscribeOutbox(authors: ["pubkey1", "pubkey2"])
...
every pubkey it mentions in `p` tags, with ``NostrRoutingAPI/publishGossip(_:strategy:)``:

```swift
let signer = EventSigner(keyPair: keyPair)
let note = try signer.signTextNote(content: "gm!", tags: [.pubkey("alice_pubkey")])
try await client.routing.publishGossip(note)
```
...
try await client.relays.add("wss://relay.example.com", config: config)
```
...
try await client.events.publish(event)

// Wait until 2 relays acknowledge
try await client.events.publish(event, strategy: .quorum(2))

// Wait for every relay to settle (accept, reject, or time out)
try await client.events.publish(event, strategy: .allSettled)
...
```swift
let result = try await client.events.publish(event)
print("accepted:", result.acceptedRelays)
print("failed:", result.failedRelays)
print("still in flight:", result.pendingRelays)
```

The convenience publish methods (``NostrEventsAPI/publishTextNote(content:tags:strategy:)``,
``NostrEventsAPI/publishReaction(to:content:strategy:)``, ...) accept the same `strategy:`
parameter and return a ``PublishedEvent`` carrying both the signed event and its
``PublishResult``:

```swift
let note = try await client.events.publishTextNote(content: "Hello!", strategy: .quorum(2))
...
``NostrRelaysAPI/connect()`` and let auto-reconnect handle drops.
...
```swift
await client.relays.clearDeduplicationCache()
```
