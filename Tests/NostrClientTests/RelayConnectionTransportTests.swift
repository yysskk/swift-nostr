...
/// In-memory ``WebSocketSession`` that lets tests drive ``RelayConnection``'s state
/// machine — connect, send, receive, publish-ack — without a live network relay.
private final class MockWebSocketSession: WebSocketSession, @unchecked Sendable {
...
private struct MockWebSocketSessionFactory: WebSocketSessionFactory {
    /// Produces the session for each connection attempt. A producer (rather than a
    /// fixed instance) lets a reconnection test hand out a fresh socket per attempt
    /// instead of sharing one mock's `sent`/`queued`/`receiveWaiters` across reconnects.
